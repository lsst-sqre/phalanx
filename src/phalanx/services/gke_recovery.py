...
class GKERecoveryService:
    ...
    def recover(self) -> None:
        ...
        This works by using Backup for GKE to back up the persistent data in
        the old cluster, then restore it to the new cluster, then using the
        Phalanx install functionality to install all of the workloads on the
        new cluster.
        """
        print("Setting all PVs to Retain in old cluster")
        self._old_cluster.retain_pvs()
        self._old_cluster.scale_down_all()

        print(
            "Backing up old cluster and restoring PVCs and PVs to new cluster"
        )
        ...
        print("Setting all PVs to Retain in new cluster")
        self._new_cluster.retain_pvs()

        self._environment.recover(
    ...
    def _check_kubectl_connect(self) -> list[str]:
        ...
        -------
        list[str]
            Any errors that this check found.
        """
        errors: list[str] = []

        try:
            print(self._old_cluster.kube_version())
            print(
        ...
        try:
            print(self._new_cluster.kube_version())
            print(
    ...
    def _check_static_ips(self) -> list[str]:
        ...
        Ignore anything in the certain namespaces because there is no way to
        declare static IPs some of the services in them. For example, the
        Weviate LoadBalancer services with the Weviate helm chart do not
        expose a value for declaring a loadBalancerIP.
        ...
        addresses = [
            address.address
            for address in self._google_cloud.list_static_ip_addresses()
        ]
        ...
        for service in services:
            if service.namespace in RECOVER_IGNORE_SERVICES:
                print(
                    ...
                    f" there is no easy way to declare loadBalancerIPs on it."
                )
                continue
    ...
    def _check_source_synced(self) -> list[str]:
        ...
        """
        errors: list[str] = []
        apps = self._source_environment.list_argocd_apps(
