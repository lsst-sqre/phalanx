...
from google.cloud.compute_v1beta import (
    Address,
    AddressesClient,
    Firewall,
    FirewallsClient,
...
__all__ = ["GoogleCloudAPIStorage"]


class GoogleCloudAPIStorage:
    ...
    ----------
    region
        The Google Cloud region to run commands against.
    project
        The Google Cloud project to run commands against.
    labels
    ...
    def __init__(
        ...
        self._backup_client = BackupForGKEClient()
        self._addresses_client = AddressesClient()
        self._firewalls_client = FirewallsClient()
        self._gke_client = ClusterManagerClient()
        self._labels = labels or {}

    def list_static_ip_addresses(self) -> list[Address]:
        """List all of the provisioned static IP addresses."""
        page_result = self._addresses_client.list(
    ...
    def create_backup_plan(self, source_cluster: str) -> str:
        ...
        operation = self._backup_client.create_backup_plan(request=request)
        result = operation.result()
        if result is None:
            raise GoogleCloudAPIError

        # If we don't do this str cast, the unit tests will fail because the
        # GRPC lib can't handle a Mock when it's expecting a string.
    ...
    def create_backup(self, backup_plan: str) -> str:
        ...
        operation = self._backup_client.create_backup(request=request)
        result = operation.result()
        if result is None:
            raise GoogleCloudAPIError
        # If we don't do this str cast, the unit tests will fail because the
    ...
    def create_restore_plan(
        ...
        operation = self._backup_client.create_restore_plan(request)
        result = operation.result()
        if result is None:
            raise GoogleCloudAPIError
        # If we don't do this str cast, the unit tests will fail because the
    ...
    def wait_for_backup(
        ...
        RetryerTimeoutError
            ...
            attempts.
        """
        condition = f"GKE backup {backup_name} completed successfully"
        for _ in retryer(condition, attempts, interval):
            backup = self._backup_client.get_backup(name=backup_name)
    ...
    def wait_for_restore(
        ...
        RetryerTimeoutError
            ...
            attempts.
        """
        condition = f"{restore_name} is successfully restored"
        for _ in retryer(condition, attempts, interval):
            restore = self._backup_client.get_restore(name=restore_name)
    ...
    def list_backup_plans(self, filter_exp: str) -> list[BackupPlan]:
        ...
        page_result = self._backup_client.list_backup_plans(request=request)
        return list(page_result)
    ...
    def list_backups(self, backup_plan: str, filter_exp: str) -> list[Backup]:
        ...
        page_result = self._backup_client.list_backups(request=request)
        return list(page_result)
    ...
    def list_restores(
        ...
        page_result = self._backup_client.list_restores(request=request)
        return list(page_result)
    ...
    def create_pvc_restore(self, restore_plan: str, backup: str) -> str:
        ...
        operation = self._backup_client.create_restore(request=request)
        result = operation.result()
        if result is None:
            raise GoogleCloudAPIError
    ...
    def list_restore_plans(self, filter_exp: str) -> list[RestorePlan]:
        ...
        page_result = self._backup_client.list_restore_plans(request=request)
        return list(page_result)
    ...
    def delete_backup_plan(self, backup_plan: str) -> None:
        ...
        operation = self._backup_client.delete_backup_plan(request)
        operation.result()
    ...
    def delete_restore_plan(self, restore_plan: str) -> None:
        ...
        operation = self._backup_client.delete_restore_plan(request)
        operation.result()
    ...
    def delete_backup(self, backup: str) -> None:
        ...
        operation = self._backup_client.delete_backup(request)
        operation.result()
    ...
    def delete_restore(self, restore: str) -> None:
        ...
        operation = self._backup_client.delete_restore(request)
        operation.result()

    def get_firewall_rule(self, name: str) -> Firewall:
        """Get information about a Google Cloud firewall rule."""
        # https://docs.cloud.google.com/python/docs/reference/google-cloud-compute-v1beta/latest/google.cloud.compute_v1beta.types.GetFirewallRequest
        request = GetFirewallRequest(firewall=name, project=self._project)
        return self._firewalls_client.get(request=request)

    def get_cluster(self, cluster: str) -> Cluster:
        """Get details about a GKE cluster."""
        # https://docs.cloud.google.com/python/docs/reference/container/latest/google.cloud.container_v1.types.GetClusterRequest.html
        name = f"{self._parent}/clusters/{cluster}"
        request = GetClusterRequest(name=name)
        return self._gke_client.get_cluster(request)
