...
The JupyterLab Controller component not only provides user lab pod management, but also subsumes the functions of the ``cachemachine`` and ``moneypenny`` applications.  That is, in addition to creating and destroying user pods and namespaces, it handles filesystem provisioning for users, and manages prepulls of cached images to local nodes.
