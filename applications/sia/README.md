...
| config.butlerDataCollections | list | `[]` | List of data (Butler) Collections The app expects each data collection to be a dictionary containing the following keys: (`config`, `label`, `repository`, `default_instrument `, `datalink_url_format` (Optional), `default`(Optional)) |
| config.butlerType | string | `"REMOTE"` | Type of butler to use (`REMOTE` or `DIRECT`) |
