...
|-----|------|---------|-------------|
| rubintv.frontend.affinity | object | `{}` | Affinity rules for the rubintv-dev frontend pod |
| rubintv.frontend.debug | bool | `false` | If set to true, enable more verbose logging. |
| rubintv.frontend.image | object | `{"pullPolicy":"IfNotPresent","repository":"ghcr.io/lsst-ts/rubintv","tag":""}` | Settings for rubintv-dev OCI image |
| rubintv.frontend.image.pullPolicy | string | `"IfNotPresent"` | Pull policy for the rubintv-dev image |
| rubintv.frontend.image.repository | string | `"ghcr.io/lsst-ts/rubintv"` | rubintv-dev frontend image to use |
| rubintv.frontend.image.tag | string | The appVersion of the chart | Tag of rubintv-dev image to use |
| rubintv.frontend.nodeSelector | object | `{}` | Node selector rules for the rubintv-dev frontend pod |
| rubintv.frontend.pathPrefix | string | `"/rubintv-dev"` | Prefix for rubintv-dev's frontend API routes. |
| rubintv.frontend.podAnnotations | object | `{}` | Annotations for the rubintv-dev frontend pod |
| rubintv.frontend.resources | object | `{}` | Resource limits and requests for the rubintv-dev frontend pod |
| rubintv.frontend.tolerations | list | `[]` | Tolerations for the rubintv-dev frontend pod |
| rubintv.fullnameOverride | string | `""` | Override the full name for resources (includes the release name) |
| rubintv.global.baseUrl | string | Set by Argo CD | Base URL for the environment |
| rubintv.global.host | string | Set by Argo CD | Host name for ingress |
| rubintv.global.tsVaultSecretsPath | string | `""` | Relative path for tsVault secrets |
| rubintv.global.vaultSecretsPath | string | Set by Argo CD | Base path for Vault secrets |
...
| rubintv.redis.affinity | object | `{}` | Affinity rules for the Redis pod |
| rubintv.redis.config.secretKey | string | `"redis-password"` | Key inside secret from which to get the Redis password (do not change) |
| rubintv.redis.config.secretName | string | `"rubintv-secrets"` | Name of secret containing Redis password (may require changing if fullnameOverride is set) |
| rubintv.redis.nodeSelector | object | `{}` | Node selection rules for the Redis pod |
| rubintv.redis.persistence.accessMode | string | `"ReadWriteOnce"` | Access mode of storage to request |
| rubintv.redis.persistence.enabled | bool | `true` | Whether to persist Redis storage and thus tokens. Setting this to false will use `emptyDir` and reset all tokens on every restart. Only use this for a test deployment. |
| rubintv.redis.persistence.size | string | `"1Gi"` | Amount of persistent storage to request |
| rubintv.redis.persistence.storageClass | string | `""` | Class of storage to request |
| rubintv.redis.persistence.volumeClaimName | string | `""` | Use an existing PVC, not dynamic provisioning. If this is set, the size, storageClass, and accessMode settings are ignored. |
| rubintv.redis.podAnnotations | object | `{}` | Pod annotations for the Redis pod |
| rubintv.redis.resources | object | See `values.yaml` | Resource limits and requests for the Redis pod |
