...
| kafka.externalListener.bootstrap.allocateLoadBalancerNodePorts | bool | true | Whether to allocate NodePort automatically for the Service with type LoadBalancer. |
...
| kafka.metricsConfig.includeTopicMetrics | bool | `false` | Include per topic metrics in Jmx Exporter |
...
| kafka.replicationFactor | int | `3` | Topic Replication Factor.  The number of copies of topic data that are maintained. |
...
| mirrormaker2.replication.offsetSyncsTopicLocation | string | source | The location (source/target) of the offset-syncs topic. |
