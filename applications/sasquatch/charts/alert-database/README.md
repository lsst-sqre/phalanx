...
| ingester.image.tag | string | `"v3.4.0"` |  |
| ingester.kafka.cluster | string | `"sasquatch"` | Name of a Strimzi Kafka cluster to connect to. |
| ingester.kafka.port | int | `9093` | Port to connect to on the Strimzi Kafka cluster. It should be an internal listener that expects TLS auth. |
...
| ingress.path | string | `"/v1"` | Subpath to host the alert database application under the ingress |
...
| server.image.tag | string | `"v3.1.0"` |  |
