...
|-----|------|---------|-------------|
| datasourceSyncer | object | `{"image":{"pullPolicy":"IfNotPresent","repository":"gcr.io/gke-release/prometheus-engine/datasource-syncer","tag":"v0.15.3-gke.0"}}` | Config for the datasource-syncer CronJob that periodically refreshes Google Cloud credentials so that the [Grafana Prometheus plugin can auth](https://cloud.google.com/stackdriver/docs/managed-prometheus/query#grafana-oauth) |
| datasourceSyncer.image | object | `{"pullPolicy":"IfNotPresent","repository":"gcr.io/gke-release/prometheus-engine/datasource-syncer","tag":"v0.15.3-gke.0"}` | The email address of the google service account to bind via Workload Identity Federation serviceAccount: |
| global.baseUrl | string | Set by Argo CD | Base URL for the environment |
| global.host | string | Set by Argo CD | Host name for ingress |
| global.vaultSecretsPath | string | Set by Argo CD | Base path for Vault secrets |
| googleCloud | string | `nil` | Config for the Google Cloud account |
| grafana | object | `{"datasource":null,"url":"http://grafana-service.grafana.svc:3000"}` | Config for Grafana (which needs to be already installed via the `grafana` Phalanx app) |
