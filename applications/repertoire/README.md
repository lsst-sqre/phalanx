...
| config.schemaSourceTemplate | string | `"https://github.com/lsst/sdm_schemas/archive/refs/tags/{version}.tar.gz"` | Template for schema artifact URLs (use {schemaVersion} placeholder) |
| config.schemaVersion | string | `"w.2025.43"` | Default schema version for all TAP services (can be overridden per-app) |
...
| config.tapServers | object | `{"livetap":{"database":"livetap","databasePasswordKey":"livetap-database-password","databaseUser":"livetap","enabled":false,"schemas":["oga_live_obscore"]},"ssotap":{"database":"ssotap","databasePasswordKey":"ssotap-database-password","databaseUser":"ssotap","enabled":false,"schemas":["dp03_1yr","dp03_10yr"]},"tap":{"database":"tap","databasePasswordKey":"tap-database-password","databaseUser":"tap","enabled":false,"schemas":["dp02_dc2","dp1","ivoa_obscore"]}}` | TAP Server configuration by application name. Configuration is used to populate & update the TAP_SCHEMA database for each enabled TAP application |
