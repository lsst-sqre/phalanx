...
| config.registrySource | string | `"file"` | Where the per-database registry catalogs come from, supplied as `MPPDB_REGISTRY_SOURCE`. `file` reads the YAML baked into the image; `clickhouse` reads `TAP_SCHEMA.registries`, published by the backend's `mppdb tapschema-ch` run, so a catalog change needs no image rebuild. Under `clickhouse` a missing or corrupt entry refuses the start rather than falling back — serving a catalog other than the published one is the failure being eliminated. |
