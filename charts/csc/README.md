...
| env | object | `{"GOTO_NUM_THREADS":1,"MKL_DOMAIN_NUM_THREADS":1,"MKL_NUM_THREADS":1,"MPI_NUM_THREADS":1,"NUMEXPR_MAX_THREADS":1,"NUMEXPR_NUM_THREADS":1,"OMP_NUM_THREADS":1,"OMP_PROC_BIND":false,"OPENBLAS_NUM_THREADS":1}` | Environment variables to set in the container. Threading and numerical library defaults are provided here and can be overridden per CSC in the application values files. |
...
| obsenv | bool | `false` | Enable observing environment sidecar? |
| obsenv_configfile | object | `{}` | This key allows specification of a json configuration file for the observing environment. If this section is used, it must contain the following attributes defined: _path_ (The container path for the configuration file), _filename_ (The configuration file name), _content_ (The json content for the configuration file) |
...
| s3Credentials | list | `[]` | This key contains a list of S3 (AWS) credential files to merge into a single aws-credentials.ini file. |
