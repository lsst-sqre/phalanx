...
| env | object | `{"GOTO_NUM_THREADS":1,"MKL_DOMAIN_NUM_THREADS":1,"MKL_NUM_THREADS":1,"MPI_NUM_THREADS":1,"NUMEXPR_MAX_THREADS":1,"NUMEXPR_NUM_THREADS":1,"OMP_NUM_THREADS":1,"OMP_PROC_BIND":false,"OPENBLAS_NUM_THREADS":1}` | Environment variables to set in the container. Threading and numerical library defaults are set here and can be overridden per CSC in the application values files. |
