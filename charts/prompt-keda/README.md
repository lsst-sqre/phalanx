...
| instrument.transferScale | int | `3` | Number of parallel processes each worker can use for transfers to and from `centralRepo` and `readRepo`. In practice, this should be set based on expected server bandwidth. |
