...
| config.bigquery.image.tag | string | `"3.11.0"` | Tag of TAP image to use |
...
| config.kafka | object | `{"enabled":false,"topics":{"jobDelete":"lsst.tap.job-delete","jobRun":"lsst.tap.job-run","jobStatus":"lsst.tap.job-status"}}` | Kafka configuration |
...
| config.pg.image.tag | string | `"1.24.0"` | Tag of TAP image to use |
...
| config.qserv.image.tag | string | `"3.11.0"` | Tag of TAP image to use |
...
| uws.image.tag | string | `"3.11.0"` | Tag of UWS database image to use |
